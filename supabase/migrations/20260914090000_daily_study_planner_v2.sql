...
-- Repair prerequisites when an earlier Lovable migration only partially ran.
...
CREATE TABLE IF NOT EXISTS public.chapter_learning_state (
  ...
  chapter_name text NOT NULL CHECK (char_length(btrim(chapter_name)) BETWEEN 1 AND 240),
  ...
  revision_minutes integer NOT NULL DEFAULT 0 CHECK (revision_minutes >= 0),
  class_minutes integer NOT NULL DEFAULT 0 CHECK (class_minutes >= 0),
...
CREATE POLICY "chapter_state_select_own" ON public.chapter_learning_state FOR SELECT TO authenticated USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "chapter_state_insert_own" ON public.chapter_learning_state;
CREATE POLICY "chapter_state_insert_own" ON public.chapter_learning_state FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
DROP POLICY IF EXISTS "chapter_state_update_own" ON public.chapter_learning_state;
CREATE POLICY "chapter_state_update_own" ON public.chapter_learning_state FOR UPDATE TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
DROP POLICY IF EXISTS "chapter_state_delete_own" ON public.chapter_learning_state;
CREATE POLICY "chapter_state_delete_own" ON public.chapter_learning_state FOR DELETE TO authenticated USING (auth.uid() = user_id);

CREATE INDEX IF NOT EXISTS chapter_state_due_idx ON public.chapter_learning_state (user_id, next_review_at);
...
BEGIN
  ...
  EXCEPTION
    WHEN insufficient_privilege OR feature_not_supported OR undefined_file THEN
      RAISE NOTICE 'pg_cron is not enabled; core planner installed without cron';
  END;
  ...
  IF to_regprocedure('cron.schedule(text,text,text)') IS NOT NULL THEN
    EXECUTE $sql$
      ...
      FROM cron.job
      WHERE jobname = 'chronodeck-daily-study-plans'
    $sql$;
    EXECUTE $sql$
    ...
    $sql$;
  ELSE
    RAISE NOTICE 'Enable pg_cron and rerun this migration to install the daily job';
  END IF;
END
$cron$;
