-- 1. Local-date helper (fixes the "day doesn't change at midnight" problem)
...
-- 2. Reading goals (reading behaves like a subject with its own daily target)
...
-- 3. Reading log accumulation, on the student's own local day
...
-- 4. Auto schedule: two-week average per active subject becomes its target
...
BEGIN
  ...
  LOOP
    ...
    -- fall back to the subject's own weekly target when there is no history yet
